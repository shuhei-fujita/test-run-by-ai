...
テストには、事前に手順と期待結果が決まっている定型的なもの（主要フローの確認、優先度の高い業務シナリオ等）と、人間の直感と判断で未知の不具合を探す探索的テスト/アドホックテストがある。現実には定型テストの実行に時間を取られ、探索的テストに十分な時間を割けないことが多い。
...
3. **実行結果は必ず人間がチェックする** — AIの解釈にはハルシネーションのリスクがあるため、全てのテスト結果に対して人間のレビューが不可欠。このプロジェクトが自動化するのはテスト手順の「実行」であり、結果の「判断」は人間が行う
4. **安定したシナリオはテストコードに昇華する** — AI実行で蓄積したコンテキスト（操作手順・セレクタ知見・画面の振る舞い）をもとにテストコードに変換できる。テストコードにすることで、実行結果の安定性向上・ハルシネーションリスクの排除・LLM利用のランニングコスト削減が得られる。playwright-cli 自体が Playwright ベースなので変換の親和性が高い

> **Note:** テスト設計（何をどうテストするか）は本プロジェクトのスコープ外。Gherkin シナリオが事前に用意されている前提で、その実行と証跡の記録を自動化する。
>
> AI駆動のテスト実行は、同じシナリオでも実行ごとに操作パスが異なる可能性がある。繰り返し同じ結果が求められるリグレッションテストは、テストコードへの変換を推奨する。
