...
Gherkin シナリオの AI 実行が安定したら、Playwright テストコード（`.spec.ts`）に変換できる。テストコードにすることで、実行の再現性向上・ハルシネーションリスク排除・LLM ランニングコスト削減が得られる。

```
> /codify-test {案件名}
```

どのシナリオを昇華するかはプロジェクトの状況（テスト頻度、外部依存の安定性、コスト等）による。`/codify-test` が対話的に選定をガイドする。
