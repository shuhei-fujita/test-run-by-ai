...
| `- [SKIP]`    | SKIP    | 前提条件不足・環境制約等でスキップ   |
| `- [BLOCKED]` | BLOCKED | 他シナリオの結果待ち・環境障害等     |
...
2. **テストデータ不足**: 必要なデータが存在しない・作成できない場合
...
- **スクリーンショット**: `test-results/{timestamp}_{案件名}/` に保存。ルート直下やtest-suites内に出力しない
...
更新内容:
- 各シナリオのチェックボックス（`- [ ]` → `- [x]` / `- [FAIL]` 等）
- 進捗サマリの数値（PASS / FAIL / SKIP / BLOCKED / 未実行）
